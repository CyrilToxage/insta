    ...
    <div class="p-4">
        <div class="flex items-center mb-4">
            <button @click="toggleLike" class="focus:outline-none">
                <svg class="w-6 h-6 transition-colors duration-200" :class="liked ? 'text-red-500' : 'text-gray-500'"
        ...
        <div class="space-y-2">
            ...
            @if ($post->comments->count() > 2)
                <a href="{{ route('posts.show', $post) }}" class="text-gray-500 text-sm">
        ...
        <form @submit.prevent="addComment" action="{{ route('comments.store', $post) }}" class="mt-4 flex space-x-2">
            @csrf
            <input type="text" name="content" placeholder="Ajouter un commentaire..."
                class="flex-1 border rounded-l px-3 py-1" required>
            <button type="submit"
                class="bg-blue-500 text-white px-4 py-1 rounded-r hover:bg-blue-600 transition-colors duration-200">
                Publier
            </button>
        </form>
    </div>
